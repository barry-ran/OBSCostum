...
static void get_duration(void *data, calldata_t *cd)
{
	...
	int64_t dur = 0;
	int64_t pts = 0;
	bool m_eof = s->media.eof;
	if (s->media.fmt)
		dur = s->media.fmt->duration;

	if (s->media.v.frame) 
		pts = (s->media.v.frame->pts *r2d(s->media.v.stream->time_base)) * 1000;

	calldata_set_int(cd, "duration", dur * 1000);
	calldata_set_int(cd, "pts", pts * 1000);
	calldata_set_bool(cd, "m_eof", m_eof);
}

static void set_frame_pos(void *data, calldata_t *cd)
{
	struct ffmpeg_source *s = data;

	float pos = (float)calldata_float(cd, "pos");
	if (s->media.fmt) {
		s->media.fmt->start_time = pos * s->media.fmt->duration;
		s->media.reset = true;
	}
}
...
static void *ffmpeg_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	proc_handler_add(ph, "void get_nb_frames(out int num_frames)",
			get_nb_frames, s);
	proc_handler_add(ph, "void set_frame_pos(out int pos)",
		set_frame_pos, s);
